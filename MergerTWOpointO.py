...
for file_index, file_name in enumerate(excel_files):
        ...
        if 'post-id' in sheet.columns:
            for old_id in sheet['post-id']:
                if pd.notnull(old_id):
                    try:
                        old_id_int = int(float(old_id))  # Handles float-looking values like 12.0
                        if old_id_int not in file_post_id_mapping:
                            file_post_id_mapping[old_id_int] = next_post_id
                            next_post_id += 1
    ...
    for sheet_name, sheet in sheets.items():
        ...
        if sheet_name_clean != 'events':
            ...
            for col in id_columns:
                sheet[col] = sheet[col].apply(
                    lambda x: file_post_id_mapping.get(int(float(x)))
                    if pd.notnull(x) and str(x).replace('.', '', 1).isdigit() and int(float(x)) in file_post_id_mapping
                    else None
                )

        # Ensure integer post-ids in 'post features'
        if sheet_name_clean == 'post features':
            for col in ['annotatorOne_post_label', 'annotatorTwo_post_label', 'annotatorThree_post_label', 'final_label']:
                if col not in sheet.columns:
                    sheet[col] = None
            if 'post-id' in sheet.columns:
                sheet['post-id'] = sheet['post-id'].astype('Int64')  # nullable integer
        ...
        # Process comments
            ...
            if 'post-id' in sheet.columns:
                sheet['post-id'] = sheet['post-id'].astype('Int64')
